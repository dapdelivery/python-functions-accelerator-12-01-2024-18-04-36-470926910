//go:build all || multicluster_outerloop || multicluster_dockerfile
// +build all multicluster_outerloop multicluster_dockerfile
...
func TestDockerFileBuiltWorkload(t *testing.T) {
	t.Log("************** TestCase START: TestDockerFileBuiltWorkload **************")
	...
	testenv.Test(t,
		...
		common_features.DockerBuildImageWithDockerFile(t, suiteConfig.Innerloop.Workload.Name, "image"),
		common_features.DockerImageTag(t, "image", outerloopConfig.DockerFileBasedWorkload.DestImageTag),
	...
	t.Log("************** TestCase END: TestDockerFileBuiltWorkload **************")
}
