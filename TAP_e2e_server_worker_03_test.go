//go:build server_worker_testing_scanning_supply_chain
// +build server_worker_testing_scanning_supply_chain
...
func TestServerWorkerOuterloopTestingScanningSupplyChain(t *testing.T) {
	t.Log("************** TestCase START: TestServerWorkerOuterloopTestingScanningSupplyChain **************")
	testenv.Test(t,
		//view context
		...
		common_features.UpdateTapProfileSupplyChain(t, suiteConfig.Tap.Name, suiteConfig.Tap.PackageName, suiteConfig.Tap.Version, "build", "testing_scanning", suiteConfig.Tap.Namespace, suiteConfig.Tap.PollTimeout),
		...
		common_features.ApplyKubectlConfigurationFile(t, serverWorkerConfig.Workload.TcpQueueDeploymentFile, outerloopConfig.Namespace),
		common_features.ApplyKubectlConfigurationFile(t, serverWorkerConfig.Workload.TcpQueueRbacFile, outerloopConfig.Namespace),

		// Define Cluster supply chain
		common_features.ApplyKubectlConfigurationFile(t, serverWorkerConfig.Workload.TcpQueueSupplyChainFile, outerloopConfig.Namespace),
		...
		common_features.CreateNamespace(t, serverWorkerConfig.Workload.ServiceInstanceNamespace),
	...
	t.Log("************** TestCase END: TestServerWorkerOuterloopTestingScanningSupplyChain **************")
}
